...
MariaDB [csd2204s18]> select first_name, last_name, department_id FROM employees WHERE department_id = (Select department_id from departments WHERE department_name = 'IT%');
Empty set (0.00 sec)
...
MariaDB [csd2204s18]> select first_name, last_name, department_id FROM employees WHERE department_id IN (Select department_id from departments WHERE department_name = 'IT%');
Empty set (0.00 sec)
...
MariaDB [csd2204s18]> select department_id FROM departments WHERE location_id IN (select location_id, country_id FROM locations WHERE country_id = 'US');
...
MariaDB [csd2204s18]> Select first_name, last_name from employees WHERE manager_id != 0 && department_id IN (select department_id FROM departments WHERE location_id IN (select location_id FROM locations WHERE country_id = 'US'));
+-------------+-------------+
| first_name  | last_name   |
+-------------+-------------+
| Alexander   | Hunold      |
...
MariaDB [csd2204s18]> select job_id, min_salary from jobs;
+------------+------------+
| job_id     | min_salary |
+------------+------------+
| AD_PRES    |      20000 |
...
MariaDB [csd2204s18]> select first_name, last_name, salary, department_id FROM employees WHERE salary > (SELECT avg(salary) FROM employees) && department_id IN(SELECT department_id FROM departments WHERE department_name LIKE 'IT%');
+------------+-----------+---------+---------------+
